...
export function createRoomBookmarkletHref(systemOrigin: string): string {
  ...
  const script = [
    ...
    "let ready=false,queued=null;const target=new URL(I).origin;",
    ...
    "const deliver=p=>{if(!ready){queued=p;return}P.postMessage({source:S,type:'RESERVATION',payload:p},target);badge.textContent='휴넷 회의실 예약 등록 확인 중'};",
