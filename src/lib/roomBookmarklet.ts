...
export function createRoomBookmarkletHref(systemOrigin: string): string {
  ...
  const script = [
    ...
    "let ready=false,sent=false,queued=null;const target=new URL(I).origin;",
    ...
    "const deliver=p=>{if(sent)return;if(!ready){queued=p;return}sent=true;P.postMessage({source:S,type:'RESERVATION',payload:p},target);badge.textContent='휴넷 회의실 예약 등록 확인 중'};",
    "addEventListener('message',e=>{if(e.origin!==target||e.data?.source!==S||e.data?.type!=='READY')return;ready=true;if(queued)deliver(queued)});",
    ...
    "const result=p=>{(Array.isArray(p)?p:[p]).filter(Boolean).forEach(deliver)};",
