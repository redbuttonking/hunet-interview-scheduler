...
export function createRoomBookmarkletHref(systemOrigin: string): string {
  ...
  const script = [
    ...
    "const date=v=>String(v||'').split('T')[0],time=v=>{const m=String(v||'').match(/T(\\d{2}:\\d{2})/);return m?m[1]:'00:00'},known={};",
    "const details=x=>x?.startTime&&x?.endTime?{date:date(x.startTime),startTime:time(x.startTime),endTime:time(x.endTime)}:{};",
    "const build=(method,url,text)=>{try{const j=JSON.parse(text);if(String(j.code)!=='200'||!j.data)return null;if((method==='POST'||method==='PUT')&&/\\/reserve(\\/\\d+)?$/.test(url)){const d=j.data,p={action:method==='POST'?'create':'update',externalId:Number(d.id),roomName:String(d.itemName||''),...details(d)};known[p.externalId]=p;return p}if(method==='DELETE'&&url.includes('/asset/item/reservation')){const a=Array.isArray(j.data)?j.data:[j.data];return a.map(x=>{const id=Number(x.id),saved=known[id],p={action:'cancel',externalId:id,roomName:String(x.itemName||x.name||saved?.roomName||''),...details(x),...(!x.startTime&&!x.endTime&&saved?{date:saved.date,startTime:saved.startTime,endTime:saved.endTime}:{})};delete known[id];return p})}}catch(_){return null}return null};",
